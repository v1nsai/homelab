...
source apps/services/secrets.env

if [ -z "$NC_HOST" ]; then
    read -sn1 -p "Enter Nextcloud host: " NC_HOST
    cat "NC_HOST=$NC_HOST" >> apps/services/secrets.env
...
kubectl create secret -n nextcloud generic nextcloud-admin \
    ...
    --dry-run=client \
    --output=yaml > apps/services/secret.yaml
kubeseal --format=yaml --cert=./.sealed-secrets.pub < apps/services/secret.yaml | tee -a apps/services/app/sealed-secrets.yaml
...
kubectl create secret generic -n nextcloud secret-values \
    ...
    --dry-run=client \
    --output yaml | \
kubeseal --cert ./.sealed-secrets.pub --format yaml | tee -a apps/services/app/sealed-secrets.yaml

rm -rf /tmp/nextcloud.key /tmp/nextcloud.crt /tmp/secret-values.yaml
rm -rf apps/services/secret.yaml